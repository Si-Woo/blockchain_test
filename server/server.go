...
var TLEDGER_SERVER_ENDPOINT = "0.0.0.0:9090"
...
func New() (*Server, error) {
	...
	router.GET("/api/v1/", server.HelloWorldGet)
	router.GET("/api/v1/getTransaction/:prefix", server.getTransactionGet)
	router.GET("/api/v1/getBlock/:prefix", server.getBlockGet)
	router.POST("/api/v1/proposeTransaction/:prefix", server.proposeTransactionPost)
...
func (s *Server) getTransactionGet(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	// reqParam := GetTransactionRequestParameter{}
	bn, _ := strconv.Atoi(r.URL.Query().Get("block_num"))
	tx, _ := strconv.Atoi(r.URL.Query().Get("tx_number"))
	reqBody.BlockNum = bn
	reqBody.TxNumber = tx
	...
	// transaction := s.getTransaction(reqParam.BlockNum, reqParam.TxNumber)
	if transaction == nil {
		w.WriteHeader(http.StatusNoContent)
		io.WriteString(w, "")
		return
